...
func (s *ProgContext) AllNEWLINE() []antlr.TerminalNode {
	return s.GetTokens(langParserNEWLINE)
}

func (s *ProgContext) NEWLINE(i int) antlr.TerminalNode {
	return s.GetToken(langParserNEWLINE, i)
}
...
func (p *langParser) Prog() (localctx IProgContext) {
	...
	for _la == langParserT__4 || _la == langParserINT {
		{
			p.SetState(4)
			p.expr(0)
		}
		...
		if _alt == 1 {
			...
			case 2:
				...
				}
				{
					...
					if !(_la == langParserT__2 || _la == langParserT__3) {
						p.GetErrorHandler().RecoverInline(p)
					} else {
						p.GetErrorHandler().ReportMatch(p)
						p.Consume()
